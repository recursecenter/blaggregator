...
NO_CONTENT_MESSAGE = 'Could not fetch feed from <a href="{url}">{url}</a>. Is the website up?'
INVALID_FEED_MESSAGE = (
    "This does not seem to be a valid feed. " "Please use your blog's feed url (not the web url)!"
)
SUGGEST_FEED_URL_MESSAGE = INVALID_FEED_MESSAGE + (' It could be -- <a href="{url}">{url}</a>')
SUCCESS_MESSAGE = (
...
def add_blog(request):
    ...
    if feed_url:
        ...
        if contents is None:  # URLError or HTTPError
        ...
        elif contents.bozo and not isinstance(
            ...
            Blog.objects.create(user=User.objects.get(id=user_id), feed_url=feed_url)
            messages.success(
                request,
...
def new(request):
    ...
    context = {"post_list": post_list, "show_avatars": True, "page_view": "new"}
